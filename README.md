...
Replacement binaries are written through an exclusively created temporary
file beside the target, then atomically renamed into place. Existing
`<target>.new` files and symlinks are left untouched. Temporary replacement
files are removed on ordinary failures; a crash can leave a hidden
`.<target>.new-*` file behind. Automatic sweeping is disabled because a
leftover cannot safely be distinguished from another update's active file.
The installation directory must be trusted against modification by other users.
